...
AR_DEFINE_RESOLVER(HybridResolver, ArResolver);
...
HybridResolver::HybridResolver() = default;

HybridResolver::~HybridResolver() = default;

std::string
HybridResolver::_CreateIdentifier(
    const std::string& assetPath,
    const ArResolvedPath& anchorAssetPath) const
{
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER).Msg("Resolver::_CreateIdentifier('%s', '%s')\n",
...
std::string
HybridResolver::_CreateIdentifierForNewAsset(
    const std::string& assetPath,
    const ArResolvedPath& anchorAssetPath) const
{
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER).Msg(
    ...
    const std::string& assetPath) const
{
    ...
    if (_IsRelativePath(assetPath)) {
        if (this->_IsContextDependentPath(assetPath)) {
            ...
            const HybridResolverContext* contexts[2] = {this->_GetCurrentContextPtr(), &_fallbackContext};
            for (const HybridResolverContext* ctx : contexts) {
                if (ctx) {
                    ...
                    if (!mappingPairs.empty()){
                        if (!ctx->GetMappingRegexExpressionStr().empty())
                        {
                            ...
                            TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContextForAsset('%s')"
...
ArResolvedPath
HybridResolver::_ResolveForNewAsset(
    const std::string& assetPath) const
{
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER).Msg("Resolver::_ResolveForNewAsset('%s')\n", assetPath.c_str());
    return ArResolvedPath(assetPath.empty() ? assetPath : TfAbsPath(assetPath));
}

ArResolverContext
HybridResolver::_CreateDefaultContext() const
{
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContext()\n");
    return _fallbackContext;
}

ArResolverContext
HybridResolver::_CreateDefaultContextForAsset(
    const std::string& assetPath) const
{
    ...
    // a HybridResolverContext, thats why we just use the fallback context.
    ...
    if(this->_GetCurrentContextObject<HybridResolverContext>() != nullptr){
        TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContextForAsset('%s') - Skipping on same stage\n", assetPath.c_str());
        return ArResolverContext(_fallbackContext);
    }
    ...
    if(map_iter != _sharedContexts.end()){
        ...
        {
            TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContextForAsset('%s') - Reusing context on different stage\n", assetPath.c_str());
            return ArResolverContext(map_iter->second.ctx);
        }else{
            TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContextForAsset('%s') - Reusing context on different stage, reloading due to changed timestamp\n", assetPath.c_str());
            map_iter->second.ctx.RefreshFromMappingFilePath();
            return ArResolverContext(map_iter->second.ctx);
        }
    }
    // Create new context
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_CreateDefaultContextForAsset('%s') - Constructing new context\n", assetPath.c_str());
    std::string assetDir = TfGetPathName(TfAbsPath(resolvedPathStr));
    struct HybridResolverContextRecord record;
    record.timestamp = this->_GetModificationTimestamp(assetPath, resolvedPath);
    record.ctx = HybridResolverContext(resolvedPath);
    _sharedContexts.insert(std::pair<std::string, HybridResolverContextRecord>(resolvedPath, record));
    return ArResolverContext(record.ctx);
}

bool
HybridResolver::_IsContextDependentPath(
    const std::string& assetPath) const
{
    ...
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_IsContextDependentPath()\n");
    return _IsSearchPath(assetPath);
}

void
HybridResolver::_RefreshContext(
    const ArResolverContext& context)
{
    TF_DEBUG(HYBRIDRESOLVER_RESOLVER_CONTEXT).Msg("Resolver::_RefreshContext()\n");
    const HybridResolverContext* ctx = this->_GetCurrentContextPtr();
    if (!ctx) {
        return;
    }
...
ArTimestamp
HybridResolver::_GetModificationTimestamp(
    const std::string& assetPath,
...
HybridResolver::_OpenAssetForWrite(
...
const HybridResolverContext* 
HybridResolver::_GetCurrentContextPtr() const
{
    return _GetCurrentContextObject<HybridResolverContext>();
}
